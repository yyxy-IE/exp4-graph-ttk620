...
void connectivityAnalysis(Graph *g) {
    ...
    for (int i = 0; i < g->vertexNum; i++) {
        if (!visited[i]) {
            componentCount++;
            printf("第 %d 个连通分量的站点：", componentCount);
            Queue *q = createQueue(g->vertexNum);
            enqueue(q, i);
            visited[i] = 1;
...
// Dijkstra 算法（数组实现，O(V²)）
void dijkstra(Graph *g, int start, int *dist, int *prev) {
    int *known = (int*)malloc(sizeof(int) * g->vertexNum);
    // 初始化
    for (int i = 0; i < g->vertexNum; i++) {
        dist[i] = INT_MAX;
        prev[i] = -1;
    ...
    for (int i = 0; i < g->vertexNum; i++) {
        ...
        while (e) {
            int v = e->adjVex;
            if (!known[v] && dist[u] != INT_MAX && dist[u] + e->weight < dist[v]) {
...
// 最少换乘路径查询（临时将边权设为1）
void shortestPathByTransfer(Graph *g, int start, int end) {
    // 无向图每条边对应两个边结点，总数为 2*edgeNum
    int totalEdgeNodes = 2 * g->edgeNum;
    int *origWeights = (int*)malloc(sizeof(int) * totalEdgeNodes);
    int idx = 0;
    ...
    if (dist[end] == INT_MAX) {
        ...
        // 换乘次数 = 边数 - 1（按实验要求公式）
        int transferCount = dist[end] > 0 ? dist[end] - 1 : 0;
        printf("\n最少换乘路径（换乘 %d 次）：", transferCount);
        printPath(g, prev, start, end);
        printf("\n");
    }
    free(dist);
